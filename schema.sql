...
USE `personal_trainer_digitale`$$
DROP TRIGGER IF EXISTS `personal_trainer_digitale`.`controllo_esistenza_esercizio` $$
USE `personal_trainer_digitale`$$
CREATE DEFINER=`root`@`localhost` TRIGGER `controllo_esistenza_esercizio` BEFORE INSERT ON `contenuto` FOR EACH ROW BEGIN
	DECLARE esiste INT;
    SELECT COUNT(*) INTO esiste FROM esercizi WHERE codice_es = NEW.codice_es;
    IF esiste = 0 THEN
        SIGNAL SQLSTATE '45005' SET MESSAGE_TEXT = 'L\'esercizio con codice non esiste.';
    END IF;
END

USE `personal_trainer_digitale`$$
DROP TRIGGER IF EXISTS `personal_trainer_digitale`.`validate_esercizi` $$
USE `personal_trainer_digitale`$$
CREATE DEFINER=`root`@`localhost` TRIGGER `validate_esercizi` BEFORE INSERT ON `esercizi` FOR EACH ROW BEGIN
	IF NEW.num_serie = 0 OR NEW.ripetizioni = 0 THEN
    SIGNAL SQLSTATE '45000'
    SET MESSAGE_TEXT = 'Serie e ripetizioni devono essere maggiori di zero.';
  END IF;
END

USE `personal_trainer_digitale`$$
DROP TRIGGER IF EXISTS `personal_trainer_digitale`.`check_esercizio_scheda_attiva` $$
USE `personal_trainer_digitale`$$
CREATE DEFINER=`root`@`localhost` TRIGGER `check_esercizio_scheda_attiva` BEFORE INSERT ON `interagisce` FOR EACH ROW BEGIN
	DECLARE scheda_attiva_id INT;
    DECLARE esercizio_in_scheda INT;

    SELECT id_scheda_attiva INTO scheda_attiva_id
    FROM scheda_attiva
    WHERE cf_atleta = NEW.cf_atleta
    LIMIT 1;

    SELECT COUNT(*) INTO esercizio_in_scheda
    FROM contenuto
    WHERE id_scheda_att = scheda_attiva_id AND codice_es = NEW.codice_es;

    IF esercizio_in_scheda = 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Esercizio non presente nella scheda attiva corrente';
    END IF;
END

USE `personal_trainer_digitale`$$
DROP TRIGGER IF EXISTS `personal_trainer_digitale`.`check_macchinario_esistente` $$
USE `personal_trainer_digitale`$$
CREATE DEFINER=`root`@`localhost` TRIGGER `check_macchinario_esistente` BEFORE INSERT ON `macchinario` FOR EACH ROW BEGIN
	DECLARE macchinario_count INT;

    SELECT COUNT(*) INTO macchinario_count
    FROM esercizi
    WHERE codice_es = NEW.codice_es;

    IF macchinario_count = 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Esercizio non esistente';
    END IF;
END

USE `personal_trainer_digitale`$$
DROP TRIGGER IF EXISTS `personal_trainer_digitale`.`archivia_scheda_vecchie` $$
USE `personal_trainer_digitale`$$
CREATE DEFINER=`root`@`localhost` TRIGGER `archivia_scheda_vecchie` BEFORE INSERT ON `scheda_attiva` FOR EACH ROW BEGIN
	DECLARE vecchio_id INT;
    DECLARE vecchia_descrizione TEXT;

    SELECT id_scheda_attiva, descrizione INTO vecchio_id, vecchia_descrizione
    FROM scheda_attiva
    WHERE cf_atleta = NEW.cf_atleta
    LIMIT 1;

    IF vecchio_id IS NOT NULL THEN
        INSERT INTO scheda_archiviata (descrizione, data_archiviazione, cf_atleta)
        VALUES (vecchia_descrizione, CURDATE(), NEW.cf_atleta);

        DELETE FROM scheda_attiva WHERE id_scheda_attiva = vecchio_id;
    END IF;
END
...
USE `personal_trainer_digitale`$$
CREATE DEFINER=`root`@`localhost` TRIGGER `verifica_durata_sessione` BEFORE INSERT ON `sessione_allenamento` FOR EACH ROW BEGIN
	 IF NEW.durata < 30 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'La durata minima di una sessione è di 30 minuti';
    END IF;
...
DROP TRIGGER IF EXISTS `personal_trainer_digitale`.`verifica_durata_sessione` $$
USE `personal_trainer_digitale`$$
CREATE TRIGGER `check_singola_interazione`
BEFORE INSERT ON `interagisce`
FOR EACH ROW
BEGIN
