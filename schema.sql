...
BEGIN
    ...
    IF NOT EXISTS (
        ...
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Esercizio non trovato';
    END IF;
    ...
    IF NOT EXISTS (
        ...
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'Scheda di allenamento non attiva o non trovata';
    END IF;

    INSERT INTO `personal_trainer_digitale`.`contenuto` (`codice_es`, `id_scheda`)
    VALUES (var_codice_es, var_id_scheda);
...
CREATE EVENT IF NOT EXISTS elimina_schede_archiviate_vecchie ON SCHEDULE EVERY 1 DAYx STARTS CURRENT_TIMESTAMP DO DELETE FROM personal_trainer_digitale.scheda_allenamento
  WHERE stato = 0
    AND data_archiviazione IS NOT NULL
    AND data_archiviazione < (CURRENT_DATE - INTERVAL 5 YEAR);
